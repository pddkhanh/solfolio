...
import { CircuitBreakerService, CircuitBreakerState, CircuitBreakerError } from './circuit-breaker.service';
...
describe('CircuitBreakerService', () => {
  ...
  describe('execute', () => {
    ...
    it('should transition to half-open after timeout', async () => {
      ...
      const options = { timeout: 100, failureThreshold: 5, successThreshold: 1 };
      ...
      expect(service.getStats('test-service')?.state).toBe(CircuitBreakerState.OPEN);

      // Wait for timeout
      await new Promise(resolve => setTimeout(resolve, 150));

      // Next call should succeed and close circuit
      const result = await service.execute('test-service', mockOperation, options);
      expect(result).toBe('success');
      expect(service.getStats('test-service')?.state).toBe(CircuitBreakerState.CLOSED);
    });
    ...
    it('should handle multiple services independently', async () => {
      ...
      expect(service.getStats('service1')?.state).toBe(CircuitBreakerState.OPEN);
      expect(service.getStats('service2')?.state).toBe(CircuitBreakerState.CLOSED);
    });
  ...
  describe('resetAll', () => {
    it('should reset all circuits', async () => {
      ...
      expect(service.getStats('service1')?.state).toBe(CircuitBreakerState.OPEN);
      expect(service.getStats('service2')?.state).toBe(CircuitBreakerState.OPEN);

      service.resetAll();

      expect(service.getStats('service1')?.state).toBe(CircuitBreakerState.CLOSED);
      expect(service.getStats('service2')?.state).toBe(CircuitBreakerState.CLOSED);
    });
  });
});